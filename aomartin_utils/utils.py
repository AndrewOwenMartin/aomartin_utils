...
def recurse_directory(root_path):
    ...
    grouper = itertools.groupby(
        sorted(pathlib.Path(root_path).iterdir(), key=is_directory), key=is_directory
    )
...
def my_latexmk(path, gobble=True):
    ...
    if gobble:
        ...
        completed_process = subprocess.run(
            cmd, capture_output=True, encoding="ISO-8859-1", env=env
        )
